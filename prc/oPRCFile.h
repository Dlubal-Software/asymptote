...
struct RGBAColour
{
  ...
  friend RGBAColour operator + (const RGBAColour& a, const RGBAColour& b)
  { return RGBAColour(a.R+b.R,a.G+b.G,a.B+b.B,a.A+b.A); }
...
struct PRCtexture
{
  ...
  */
  ...
  /*
    PRC_TEXTURE_APPLYING_MODE_NONE
    PRC_TEXTURE_APPLYING_MODE_LIGHTING
    PRC_TEXTURE_APPLYING_MODE_ALPHATEST
    PRC_TEXTURE_APPLYING_MODE_VERTEXCOLOR
  */
...
public:
  ...
  bool check_uniqueness; // check uniqueness of added meshes

  PRCoptions(double compression=0.0, double granularity=0.0, bool closed=false,
             bool tess=false, bool do_break=true, bool no_break=false, double crease_angle=25.8419, bool check_uniqueness=false)
...
class PRCFileStructure : public PRCStartHeader
{
  public:
    ...
    PRCLinePatternList linepatterns;
    PRCLinePatternMap linepatternMap;
    PRCStyleList styles;
    ...
    ~PRCFileStructure () {
      ...
      for(PRCLinePatternList::iterator       it=linepatterns.begin();        it!=linepatterns.end();              ++it) delete *it;
...
uint32_t add##prctype##Unique( PRC##prctype*& p##prctype);
  ...
  ADD_ADDUNIQ( LinePattern        )
  ADD_ADDUNIQ( Style              )

#undef ADD_ADDUNIQ

    uint32_t addRgbColor(double r, double g, double b);
    uint32_t addRgbColor(const PRCRgbColor& c)
    { return addRgbColor(c.red, c.green, c.blue); }
    uint32_t addRgbColorUnique(double r, double g, double b);
    uint32_t addRgbColorUnique(const PRCRgbColor& c)
    { return addRgbColorUnique(c.red, c.green, c.blue); }
    ...
    uint32_t add3DTessUnique(PRC3DTess*& p3DTess);
    uint32_t add3DWireTess(PRC3DWireTess*& p3DWireTess);
    uint32_t add3DWireTessUnique(PRC3DWireTess*& p3DWireTess);
...
class oPRCFile
{
  public:
    ...
    uint32_t addTextureDefinition(const PRCtexture& t);
    uint32_t addTexturedMaterial(const PRCmaterial &material, uint32_t n=0, const PRCtexture* const* tt=NULL);
    uint32_t addTexturedMaterial(const PRCmaterial &material, uint32_t n=0, const uint32_t * const tt=NULL);
    ...
    uint32_t createLines(uint32_t nP, const double P[][3], uint32_t nI, const uint32_t PI[],
                      bool segment_color, uint32_t nC, const RGBAColour C[], uint32_t nCI, const uint32_t CI[], bool cu);
    uint32_t createLine(uint32_t nP, const double P[][3], uint32_t nI, const uint32_t PI[],
                      bool segment_color, uint32_t nC, const RGBAColour C[], const uint32_t CI[], bool cu);
    uint32_t createLineSegments(uint32_t nP, const double P[][3], uint32_t nI, const uint32_t PI[][2],
                               uint32_t nC, const RGBAColour C[], const uint32_t CI[][2], const uint32_t SCI[], bool cu);
    uint32_t createLineTriangles(uint32_t nP, const double P[][3], uint32_t nI, const uint32_t PI[][3],
                            bool segment_color, uint32_t nC, const RGBAColour C[], const uint32_t CI[][3], bool cu);
    uint32_t createLineQuads(uint32_t nP, const double P[][3], uint32_t nI, const uint32_t PI[][4],
    ...
    uint32_t createTriangleMesh(uint32_t nP, const double P[][3], uint32_t nI, const uint32_t PI[][3], const PRCmaterial& m,
                      ...
                      uint32_t nM, const PRCmaterial M[], const uint32_t MI[], double ca, bool cu)
            {
               ...
               if(M!=NULL && nM!=0 && MI!=NULL)
               {
                 uint32_t* const styles = new uint32_t[nI];
                 for(uint32_t i=0; i<nI; i++)
                   styles[i]=addMaterial(M[MI[i]]);
                 const uint32_t meshid =  createTriangleMesh(nP, P, nI, PI, style, nN, N, NI, nT, T, TI, nC, C, CI, styles, ca, cu);
                 delete[] styles;
                 return meshid;
               }
               else
                 return createTriangleMesh(nP, P, nI, PI, style, nN, N, NI, nT, T, TI, nC, C, CI, NULL, ca, cu);
            }
    ...
    uint32_t createQuadMesh(uint32_t nP, const double P[][3], uint32_t nI, const uint32_t PI[][4], const PRCmaterial& m,
                      ...
                      uint32_t nM, const PRCmaterial M[], const uint32_t MI[], double ca, bool cu)
            {
               ...
               if(M!=NULL && nM!=0 && MI!=NULL)
               {
                 ...
                 for(uint32_t i=0; i<nI; i++)
                   styles[i]=addMaterial(M[MI[i]]);
                 const uint32_t meshid =  createQuadMesh(nP, P, nI, PI, style, nN, N, NI, nT, T, TI, nC, C, CI, styles, ca, cu);
                 delete[] styles;
                 return meshid;
               }
               else
                 return createQuadMesh(nP, P, nI, PI, style, nN, N, NI, nT, T, TI, nC, C, CI, NULL, ca, cu);
            }
...
//  void addTriangle(const double P[][3], const double T[][2], uint32_t style_index);
    ...
    void addSquare(const PRCmaterial& m, PRCTRANSFORM);
...
#undef ADD_ADDUNIQ
    ...
    uint32_t addRgbColor(const PRCRgbColor& c,
       uint32_t fileStructure=0)
      {
        return fileStructures[fileStructure]->addRgbColor(c.red, c.green, c.blue);
      }
    uint32_t addRgbColorUnique(double r, double g, double b,
       uint32_t fileStructure=0)
      {
    ...
    uint32_t addRgbColorUnique(const PRCRgbColor& c,
       uint32_t fileStructure=0)
      {
        return fileStructures[fileStructure]->addRgbColorUnique(c.red, c.green, c.blue);
      }
    ...
    uint32_t add3DTessUnique(PRC3DTess*& p3DTess, uint32_t fileStructure=0)
      {
        return fileStructures[fileStructure]->add3DTessUnique(p3DTess);
      }
    ...
    uint32_t add3DWireTessUnique(PRC3DWireTess*& p3DWireTess, uint32_t fileStructure=0)
      {
        return fileStructures[fileStructure]->add3DWireTessUnique(p3DWireTess);
      }
